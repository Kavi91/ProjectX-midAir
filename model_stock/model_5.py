...
class CrossModalAttention(nn.Module):
    def __init__(self, query_channels, key_value_channels, num_heads=8, reduction=16):
        ...
        self.key = nn.Conv2d(key_value_channels, query_channels, kernel_size=1)
    ...
    def forward(self, query_features, key_features, value_features):
        ...
        K = self.key(key_features).view(B, self.num_heads, self.head_dim, H * W)
        V = self.value(value_features).view(B, self.num_heads, self.head_dim, H * W)
...
class StereoAdaptiveVO(DeepVO):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        ...
        # Depth processing (simple CNN, trained from scratch, outputting 512 channels)
        ...
        self.depth_conv4 = conv(self.batchNorm, 128, 512, kernel_size=3, stride=2, dropout=par.conv_dropout[4])
        
        # LiDAR processing (simple CNN, trained from scratch, outputting 512 channels)
        if par.enable_lidar:
            self.lidar_conv1 = conv(self.batchNorm, 10, 64, kernel_size=7, stride=2, dropout=par.conv_dropout[0])
            self.lidar_conv2 = conv(self.batchNorm, 64, 128, kernel_size=5, stride=2, dropout=par.conv_dropout[1])
            self.lidar_conv3 = conv(self.batchNorm, 128, 256, kernel_size=5, stride=2, dropout=par.conv_dropout[2])
            self.lidar_conv4 = conv(self.batchNorm, 256, 512, kernel_size=3, stride=2, dropout=par.conv_dropout[4])
        else:
            self.lidar_conv1 = None
            self.lidar_conv2 = None
            self.lidar_conv3 = None
            self.lidar_conv4 = None
        ...
        depth_channels = __tmp_depth.size(1) if par.enable_depth else 0  # 512
        ...
        if par.enable_lidar:
            ...
            lidar_channels = __tmp_lidar.size(1)  # 512
    ...
    def encode_lidar(self, x):
        if not par.enable_lidar:
            return None
        out = self.lidar_conv1(x)
        out = self.lidar_conv2(out)
        out = self.lidar_conv3(out)
        out = self.lidar_conv4(out)
        return out
