...
class CrossModalAttention(nn.Module):
    def __init__(self, query_channels, key_value_channels, num_heads=8, reduction=16):
        ...
        assert query_channels > 0, f"query_channels must be greater than 0, got {query_channels}"
        assert key_value_channels > 0, f"key_value_channels must be greater than 0, got {key_value_channels}"
...
class AdaptiveGatedAttentionFusion(nn.Module):
    def __init__(self, rgb_channels, depth_channels, lidar_channels=0, reduction=16):
        ...
        # Only create RGB-related attention if rgb_channels > 0
        ...
        # Create depth-related attention if depth_channels > 0
        ...
        # Create LiDAR-related attention if lidar_channels > 0
        self.lidar_to_rgb = CrossModalAttention(query_channels=lidar_channels, key_value_channels=rgb_channels, num_heads=8, reduction=reduction) if lidar_channels > 0 and rgb_channels > 0 else None
        self.lidar_to_depth = CrossModalAttention(query_channels=lidar_channels, key_value_channels=depth_channels, num_heads=8, reduction=reduction) if lidar_channels > 0 and depth_channels > 0 else None
        ...
        # Gating mechanism
        # Adjust the number of gates based on active modalities
        num_gates = sum(1 for x in [rgb_channels, depth_channels, lidar_channels] if x > 0)
        self.gate = nn.Sequential(
            nn.Conv2d(self.total_channels + num_gates, num_gates, kernel_size=1),
            nn.Sigmoid()
        ) if num_gates > 0 else None
    ...
    def forward(self, rgb_features, depth_features, lidar_features=None):
        ...
        if rgb_features is not None and depth_features is not None and rgb_features.size()[2:] != depth_features.size()[2:]:
        ...
        if lidar_features is not None:
            if rgb_features is not None and rgb_features.size()[2:] != lidar_features.size()[2:]:
                lidar_features = F.interpolate(lidar_features, size=rgb_features.size()[2:], mode='bilinear', align_corners=False)
            elif depth_features is not None and depth_features.size()[2:] != lidar_features.size()[2:]:
                lidar_features = F.interpolate(lidar_features, size=depth_features.size()[2:], mode='bilinear', align_corners=False)

        # Cross-modal attention
        # RGB attends to depth and LiDAR
        rgb_cross = rgb_features
        if self.rgb_to_depth is not None and rgb_features is not None and depth_features is not None:
            rgb_cross = rgb_cross + self.rgb_to_depth(rgb_features, depth_features, depth_features)
        if self.rgb_to_lidar is not None and rgb_features is not None and lidar_features is not None:
        ...
        if self.depth_to_rgb is not None and depth_features is not None and rgb_features is not None:
            depth_cross = depth_cross + self.depth_to_rgb(depth_features, rgb_features, rgb_features)
        if self.depth_to_lidar is not None and depth_features is not None and lidar_features is not None:
        ...
        if lidar_features is not None:
            if self.lidar_to_rgb is not None and rgb_features is not None:
                lidar_cross = lidar_cross + self.lidar_to_rgb(lidar_features, rgb_features, rgb_features)
            if self.lidar_to_depth is not None and depth_features is not None:
                lidar_cross = lidar_cross + self.lidar_to_depth(lidar_features, depth_features, depth_features)

        # SE blocks (channel-wise attention)
        rgb_refined = rgb_cross * self.se_rgb(rgb_cross) if self.se_rgb is not None and rgb_cross is not None else rgb_cross
        depth_refined = depth_cross * self.se_depth(depth_cross) if self.se_depth is not None and depth_cross is not None else depth_cross
        lidar_refined = lidar_cross * self.se_lidar(lidar_cross) if self.se_lidar is not None and lidar_cross is not None else lidar_cross

        # Quality metrics for gating
        quality_metrics = []
        ...
        if lidar_features is not None:
            lidar_sparsity = (lidar_features == 0).float().mean(dim=[1, 2, 3], keepdim=True)
            quality_metrics.append(lidar_sparsity)
        if quality_metrics:
            quality_metrics = torch.cat(quality_metrics, dim=1)
            # Match spatial dimensions to features
            if rgb_refined is not None:
                quality_metrics = quality_metrics.expand(-1, -1, rgb_refined.size(2), rgb_refined.size(3))
            elif depth_refined is not None:
                quality_metrics = quality_metrics.expand(-1, -1, depth_refined.size(2), depth_refined.size(3))
            elif lidar_refined is not None:
                quality_metrics = quality_metrics.expand(-1, -1, lidar_refined.size(2), lidar_refined.size(3))

        # Concatenate features for gating
        features_to_concat = []
        if rgb_refined is not None:
            features_to_concat.append(rgb_refined)
        if depth_refined is not None:
            features_to_concat.append(depth_refined)
        if lidar_refined is not None:
            features_to_concat.append(lidar_refined)
        combined = torch.cat(features_to_concat, dim=1) if features_to_concat else None

        # Gating
        if self.gate is not None and combined is not None:
        ...
        gate_idx = 0
        if rgb_refined is not None:
            rgb_weighted = rgb_refined * gate_weights[:, gate_idx:gate_idx+1] if gate_weights is not None else rgb_refined
            gate_idx += 1
        else:
            rgb_weighted = None
        if depth_refined is not None:
            depth_weighted = depth_refined * gate_weights[:, gate_idx:gate_idx+1] if gate_weights is not None else depth_refined
            gate_idx += 1
        else:
            depth_weighted = None
        if lidar_refined is not None:
            lidar_weighted = lidar_refined * gate_weights[:, gate_idx:gate_idx+1] if gate_weights is not None else lidar_refined
        else:
            lidar_weighted = None

        # Final fusion
        weighted_features = []
        if rgb_weighted is not None:
            weighted_features.append(rgb_weighted)
        if depth_weighted is not None:
            weighted_features.append(depth_weighted)
        if lidar_weighted is not None:
            weighted_features.append(lidar_weighted)
        fused_features = self.fusion(torch.cat(weighted_features, dim=1)) if weighted_features else None
...
class StereoAdaptiveVO(DeepVO):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        ...
        # Depth processing with pretrained Monodepth2 ResNet18 encoder
        if par.enable_depth:
            # Load pretrained ResNet18
            resnet = resnet18()
            # Load Monodepth2 weights
            monodepth2_checkpoint = torch.load('/home/krkavinda/ProjectX-RGBDL-C-ATTN/MonoDepth_Models/encoder.pth', map_location='cpu', weights_only=True)
            # Map Monodepth2 state dict keys to ResNet18 keys
            state_dict = {}
            for name, param in monodepth2_checkpoint.items():
                new_name = name.replace('encoder.', '')  # Remove 'encoder.' prefix if present
                state_dict[new_name] = param
            resnet.load_state_dict(state_dict, strict=False)
            # Adapt the first layer to handle 2 input channels (stereo depth)
            resnet.conv1 = nn.Conv2d(2, 64, kernel_size=7, stride=2, padding=3, bias=False)
            # Initialize the new conv1 weights by copying the pretrained weights for the first 2 channels
            with torch.no_grad():
                pretrained_weights = resnet.conv1.weight.clone()
                resnet.conv1.weight[:, :2, :, :] = pretrained_weights[:, :2, :, :]  # Copy first 2 channels
                resnet.conv1.weight[:, 2:, :, :] = pretrained_weights[:, :1, :, :]  # Repeat for remaining channels
            # Remove the fully connected layer and avgpool, keep only the convolutional layers
            ...
            # Adapt the output to 256 channels to match the original depth branch
            self.depth_adapt = nn.Conv2d(512, 256, kernel_size=1, stride=1)
        else:
            self.depth_backbone = None
            self.depth_adapt = None
        
        # LiDAR processing with pretrained Lorcon-LO
        if par.enable_lidar:
            # Define the Lorcon-LO feature extraction backbone
            self.lidar_backbone = nn.Sequential(
                # simple_conv1
                nn.Conv2d(10, 64, kernel_size=7, stride=2, padding=3, bias=True),
                nn.BatchNorm2d(64),
            ...
            for name, param in state_dict.items():
                # Include only feature extraction layers (exclude RNN, FC, etc.)
                if 'simple_conv' in name or 'conv_bn' in name:
                    # Map Lorcon-LO keys to our backbone keys
                    new_name = name.replace('simple_', '')  # e.g., 'simple_conv1' -> 'conv1'
                    new_name = new_name.replace('conv_bn', 'bn')  # e.g., 'conv_bn1' -> 'bn1'
                    backbone_state_dict[new_name] = param

            # Load the weights into the backbone
            self.lidar_backbone.load_state_dict(backbone_state_dict, strict=False)

            # Adapt the output to 128 channels to match the original LiDAR branch
        ...
        if par.enable_rgb:
            ...
            rgb_channels = __tmp_rgb.size(1) * 2  # 2048
            ...
            rgb_h, rgb_w = 3, 10  # Default values if RGB is disabled
        ...
        if par.enable_lidar:
            ...
            lidar_h, lidar_w = depth_h, depth_w  # Match depth dimensions for fusion
        ...
        __tmp_depth_resized = torch.zeros(1, depth_channels, depth_h, depth_w) if depth_channels > 0 else None
        __tmp_lidar_resized = torch.zeros(1, lidar_channels, lidar_h, lidar_w) if par.enable_lidar else None
    ...
    def encode_depth(self, x):
        out = self.depth_backbone(x)  # [batch_size * new_seq_len, 512, H', W']
        out = self.depth_adapt(out)  # [batch_size * new_seq_len, 256, H', W']
        return out

    def encode_lidar(self, x):
        if not par.enable_lidar:
            return None
        out = self.lidar_backbone(x)  # [batch_size * (new_seq_len + 1), 512, H', W']
    ...
    def forward(self, x):
        ...
        if x_depth.size(1) < 2 or (par.enable_lidar and x_lidar.size(1) < 2):
            raise ValueError(f"Sequence length too short: Depth={x_depth.size(1)}, LiDAR={x_lidar.size(1) if par.enable_lidar else 'N/A'}")
        
        # Process RGB if enabled
        rgb_features = None
        if par.enable_rgb:
            ...
            batch_size = x_depth.size(0)
            new_seq_len = x_depth.size(1) - 1  # Since depth is paired
